...
from FRCNN.FRCNN_PyTorch.datasets import voc
from FRCNN.FRCNN_PyTorch.datasets import image
from FRCNN.FRCNN_PyTorch.models.faster_rcnn import FasterRCNNModel
from FRCNN.FRCNN_PyTorch.models import vgg16
from FRCNN.FRCNN_PyTorch.models import vgg16_torch
from FRCNN.FRCNN_PyTorch.models import resnet
from FRCNN.FRCNN_PyTorch.statistics import TrainingStatistics
from FRCNN.FRCNN_PyTorch.statistics import PrecisionRecallCurveCalculator
from FRCNN.FRCNN_PyTorch import state
from FRCNN.FRCNN_PyTorch import utils
from FRCNN.FRCNN_PyTorch import visualize
from FRCNN.FRCNN_PyTorch.models.backbone import Backbone
...
def enable_cuda_memory_profiler(model):
    from FRCNN_PyTorch import profile
    import sys
    import threading
...
def predict_video(model):
    ...
    while True:
        ...
        fps = int(fps)
        print(fps)
        #Processing on camera frame
        image_data = t.from_numpy(frame).unsqueeze(dim = 0)
        transpose_image_data = np.transpose(image_data,(0,3,1,2)).cuda()
        new_image_data = transpose_image_data.type(t.float32).cuda()
        scored_boxes_by_class_index = model.predict(image_data = new_image_data, score_threshold = 0.3)
        for class_index, scored_boxes in scored_boxes_by_class_index.items():
            for i in range(scored_boxes.shape[0]):
                ...
                cv2.rectangle(frame, (int(scored_box[1]), int(scored_box[0])), (int(scored_box[3]), int(scored_box[2])), color = (255, 0, 0), thickness = 2)
                cv2.putText(frame, text, (int(scored_box[1]), int(scored_box[0])), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color = (255, 0, 0), thickness = 1)
...
if __name__ == "__main__":
    ...
    group.add_argument("--predict-video", action = "store_true", help = "Run inference on cv2 video")
    ...
    elif options.predict_video:
        predict_video(model = model)
