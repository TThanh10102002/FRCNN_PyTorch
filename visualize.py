...
def show_detection_on_camera(frame, scored_boxes_by_class_index, class_index_to_name):
    color_idx = 0
    for class_index, scored_boxes in scored_boxes_by_class_index.items():
        for i in range(scored_boxes.shape[0]):
            scored_box = scored_boxes[i,:]
            ...
            text = "%s %1.2f" % (class_name, scored_box[4])
            color = _class_to_color(class_index = class_index)
            cv2.rectangle(frame, (scored_box[1], scored_box[0]), (scored_box[3], scored_box[2]), color = color, thickness = 2)
            cv2.putText(frame, text, (scored_box[1], scored_box[0]), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color = color, thickness = 1.5)
